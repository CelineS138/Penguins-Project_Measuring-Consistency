...
forwards <- forwards |> 
  ungroup() |> 
  mutate(qrt1 = quantile(total_zscore, 0.25),
         qrt2 = quantile(total_zscore, 0.5),
         qrt3 = quantile(total_zscore, 0.75))



forwards |> 
  ggplot(aes(as.factor(start_year), total_zscore, label = Player)) +
  geom_violin(fill = "#FCB514") + 
  geom_hline(aes(yintercept = qrt1)) +
  geom_hline(aes(yintercept = qrt2)) +
  geom_hline(aes(yintercept = qrt3)) +
...
long_forwards <- long_forwards |>
  mutate(
    ...
    zscore_wmc = (
      if_else(!is.na(GP_2016) & !is.na(GP_2017) & !is.na(`total_zscore_2016`) & !is.na(`total_zscore_2017`), (GP_2016 + GP_2017) * abs(`total_zscore_2016` - `total_zscore_2017`), 0) +
        if_else(!is.na(GP_2017) & !is.na(GP_2018) & !is.na(`total_zscore_2017`) & !is.na(`total_zscore_2018`), (GP_2017 + GP_2018) * abs(`total_zscore_2017` - `total_zscore_2018`), 0) +
        if_else(!is.na(GP_2018) & !is.na(GP_2019) & !is.na(`total_zscore_2018`) & !is.na(`total_zscore_2019`), (GP_2018 + GP_2019) * abs(`total_zscore_2018` - `total_zscore_2019`), 0) +
        if_else(!is.na(GP_2019) & !is.na(GP_2020) & !is.na(`total_zscore_2019`) & !is.na(`total_zscore_2020`), (GP_2019 + GP_2020) * abs(`total_zscore_2019` - `total_zscore_2020`), 0) +
        if_else(!is.na(GP_2020) & !is.na(GP_2021) & !is.na(`total_zscore_2020`) & !is.na(`total_zscore_2021`), (GP_2020 + GP_2021) * abs(`total_zscore_2020` - `total_zscore_2021`), 0) +
        if_else(!is.na(GP_2021) & !is.na(GP_2022) & !is.na(`total_zscore_2021`) & !is.na(`total_zscore_2022`), (GP_2021 + GP_2022) * abs(`total_zscore_2021` - `total_zscore_2022`), 0) +
        if_else(!is.na(GP_2022) & !is.na(GP_2023) & !is.na(`total_zscore_2022`) & !is.na(`total_zscore_2023`), (GP_2022 + GP_2023) * abs(`total_zscore_2022` - `total_zscore_2023`), 0)
    ) / 
      (
        sum(if_else(!is.na(GP_2016) & !is.na(GP_2017), GP_2016 + GP_2017, 0), 
            if_else(!is.na(GP_2017) & !is.na(GP_2018), GP_2017 + GP_2018, 0), 
...
long_forwards <- long_forwards |> 
  mutate(mean_zscore = mean_zscore_2016,
         ...
         med_mean_zscore = median(mean_zscore, na.rm = TRUE),
         med_zscore_wmc = median(zscore_wmc, na.rm = TRUE),
...
long_forwards |> 
  ggplot(aes(mean_zscore, zscore_wmc, label = Player)) +
  geom_point(alpha = 0.7) +
  geom_vline(aes(xintercept = med_mean_zscore), linetype = "dashed", size = 1, color = "#FCB514") +
  geom_hline(aes(yintercept = med_zscore_wmc), linetype = "dashed", size = 1, color = "#FCB514") +
  geom_label_repel(size = 2.5, box.padding = 0.4) +
  theme_light() +
  labs(
    title = "Combined Clustering of Good vs. Bad Players \nand Consistent vs. Inconsistent Players",
...
#   geom_vline(aes(xintercept = med_total_zscore), linetype = "dashed", size = 1) +
#   geom_hline(aes(yintercept = med_total_wmc), linetype = "dashed", size = 1) +
...
long_forwards |> 
  ...
  geom_point(alpha = 0.7) +
  geom_vline(aes(xintercept = med_EVG_zscore), linetype = "dashed", size = 1, color = "#FCB514") +
  geom_hline(aes(yintercept = med_EVG_wmc), linetype = "dashed", size = 1, color = "#FCB514") +
  geom_label_repel(size = 2.5, box.padding = 0.35) + 
  labs(
    title = "EVG Clustering of Good vs. Bad Players \nand Consistent and Inconsistent Players",
...
long_forwards |> 
  ...
  geom_point(alpha = 0.7) +
  geom_vline(aes(xintercept = med_EVA_zscore), linetype = "dashed", size = 1, color = "#FCB514") +
  geom_hline(aes(yintercept = med_EVA_wmc), linetype = "dashed", size = 1, color = "#FCB514") +
  geom_label_repel(size = 2.5, box.padding = 0.4) +
  labs(
    title = "EVA Clustering of Good vs. Bad Players \nand Consistent and Inconsistent Players",
...
long_forwards |> 
  ...
  geom_point(alpha = 0.7) +
  geom_vline(aes(xintercept = `med_S%_zscore`), linetype = "dashed", size = 1, color = "#FCB514") +
  geom_hline(aes(yintercept = `med_S%_wmc`), linetype = "dashed", size = 1, color = "#FCB514") +
  geom_label_repel(size = 2.5, box.padding = 0.4) +
  labs(
    title = "S% Clustering of Good vs. Bad Players \nand Consistent and Inconsistent Players",
...
```{r}
contracts <- read_csv("~/CMU Summer Research/Capstone Project/Data_Copy/NHL_contracts.csv") |> 
  select(Player, Cap, Term)
...
long_forwards |> 
  filter(mean_zscore >= quantile(mean_zscore, 0.90, na.rm = TRUE)) |> 
  select(Player, mean_zscore, zscore_wmc) |> 
  arrange(zscore_wmc) |> 
  rename(Mean_Total_zscore = mean_zscore,
         Weighted_Mean_Change = zscore_wmc) |> 
  head(10)



`top5_good_&_con` <- long_forwards |> 
  # na.omit() |> 
  filter(mean_zscore >= quantile(mean_zscore, 0.80, na.rm = TRUE)) |> 
  select(Player, mean_zscore, zscore_wmc, Cap) |> 
  rename(Mean_Total_zscore = mean_zscore, 
         Weighted_Means_Change = zscore_wmc,
         Salary = Cap) |> 
  arrange(Weighted_Means_Change) |> 
  head(10)

`bottom5_good_&_incon` <- long_forwards |> 
  # na.omit() |> 
  filter(mean_zscore >= quantile(mean_zscore, 0.80, na.rm = TRUE)) |> 
  select(Player, mean_zscore, zscore_wmc, Cap) |> 
  rename(Mean_Total_zscore = mean_zscore, 
         Weighted_Means_Change = zscore_wmc) |> 
  arrange(desc(Weighted_Means_Change)) |> 
  head(5)


long_forwards |> 
  ggplot(aes(Cap, mean_zscore)) +
  geom_point() +
  geom_smooth(method = lm)

long_forwards |> 
  ggplot(aes(Cap, zscore_wmc)) +
  geom_point() +
  geom_smooth(method = lm)


cap_zscore <- summary(lm(long_forwards$mean_zscore ~ long_forwards$Cap))$r.squared

cap_wmc_r2 <- summary(lm(long_forwards$zscore_wmc ~ long_forwards$Cap))$r.squared

cap_cor_table <- data.frame(cap_wmc_r2, cap_zscore, row.names = "Adjusted R Squared") |> 
  rename(Cap_WMC = cap_wmc_r2,
         Cap_Zscore = cap_zscore)

cap_cor_table <- t(cap_cor_table) |> 
  data.frame()

cap_cor_table <- cap_cor_table |> 
  mutate(Variable1 = "Salary") |> 
  mutate(Variable2 = "V")

cap_cor_table[1, 3] = "Consistency"
cap_cor_table[2, 3] = "Performance"


cap_cor_table

kable(cap_cor_table, caption = "Correlation Table Between Salary and Consistency & Salary and Performance")

kable(`top5_good_&_con`, caption = "Top 5 Players in Offensive Performance and Consistency")
```
...
chosen <- sample(unique(long_forwards$Player), 200)

sel_long_forwards <- subset(long_forwards, Player %in% chosen)

sel_long_forwards <- sel_long_forwards |> 
  drop_na()

sel_long_forwards |> 
  ggplot(aes(`EVG/60_2023`, `EVG/60_2022`)) +
  geom_point() +
  geom_smooth(method = lm) +
  geom_abline(slope = 1) +
  theme_light() +
  labs(
    x = "Goals Per 60 at EV in 2023-24 Season",
    y = "Goals Per 60 at EV in 2022-23 Season",
    caption = "Data Courtesy of Hockey Reference",
    title = "Metric Stickiness of Goals/60 (EV) from 2023-24 and 2022-23 Seasons"
  )

summary(lm(sel_long_forwards$`EVG/60_2023` ~ sel_long_forwards$`EVG/60_2022`))$r.squared
summary(lm(sel_long_forwards$`EVG/60_2022` ~ sel_long_forwards$`EVG/60_2021`))$r.squared
summary(lm(sel_long_forwards$`EVG/60_2021` ~ sel_long_forwards$`EVG/60_2020`))$r.squared
summary(lm(sel_long_forwards$`EVG/60_2020` ~ sel_long_forwards$`EVG/60_2019`))$r.squared
summary(lm(sel_long_forwards$`EVG/60_2019` ~ sel_long_forwards$`EVG/60_2018`))$r.squared
summary(lm(sel_long_forwards$`EVG/60_2018` ~ sel_long_forwards$`EVG/60_2017`))$r.squared
summary(lm(sel_long_forwards$`EVG/60_2017` ~ sel_long_forwards$`EVG/60_2016`))$r.squared

Response = c(2023, 2022, 2021, 2020, 2019, 2018, 2017)
Explanatory = c(2022, 2021, 2020, 2019, 2018, 2017, 2016)
r_sqr = c(0.1619101, 0.1289881, 0.1877242, 0.06931722, 0.2140173, 0.1789242, 0.04607436)


EVG_rsqr_table <- data.frame(Response, Explanatory, r_sqr) |> 
  rename(`EVG_R^2` = r_sqr)



sel_long_forwards |> 
  ggplot(aes(`EVA/60_2023`, `EVA/60_2022`)) +
  geom_point() +
  geom_smooth(method = lm) +
  geom_abline(slope = 1)

summary(lm(sel_long_forwards$`EVA/60_2023` ~ sel_long_forwards$`EVA/60_2022`))$r.squared
summary(lm(sel_long_forwards$`EVA/60_2022` ~ sel_long_forwards$`EVA/60_2021`))$r.squared
summary(lm(sel_long_forwards$`EVA/60_2021` ~ sel_long_forwards$`EVA/60_2020`))$r.squared
summary(lm(sel_long_forwards$`EVA/60_2020` ~ sel_long_forwards$`EVA/60_2019`))$r.squared
summary(lm(sel_long_forwards$`EVA/60_2019` ~ sel_long_forwards$`EVA/60_2018`))$r.squared
summary(lm(sel_long_forwards$`EVA/60_2018` ~ sel_long_forwards$`EVA/60_2017`))$r.squared
summary(lm(sel_long_forwards$`EVA/60_2017` ~ sel_long_forwards$`EVA/60_2016`))$r.squared


r_sqr2 = c(0.1667251, 0.271814, 0.2336904, 0.3530796, 0.2272132, 0.2164871, 0.09711165)


EVA_rsqr_table <- data.frame(Response, Explanatory, r_sqr2) |> 
  rename(`EVA_R^2` = r_sqr2)



sel_long_forwards |> 
  ggplot(aes(`S%_2023`, `S%_2022`)) +
  geom_point() +
  geom_smooth(method = lm) +
  geom_abline(slope = 1)

summary(lm(sel_long_forwards$`S%_2023` ~ sel_long_forwards$`S%_2022`))$r.squared
summary(lm(sel_long_forwards$`S%_2022` ~ sel_long_forwards$`S%_2021`))$r.squared
summary(lm(sel_long_forwards$`S%_2021` ~ sel_long_forwards$`S%_2020`))$r.squared
summary(lm(sel_long_forwards$`S%_2020` ~ sel_long_forwards$`S%_2019`))$r.squared
summary(lm(sel_long_forwards$`S%_2019` ~ sel_long_forwards$`S%_2018`))$r.squared
summary(lm(sel_long_forwards$`S%_2018` ~ sel_long_forwards$`S%_2017`))$r.squared
summary(lm(sel_long_forwards$`S%_2017` ~ sel_long_forwards$`S%_2016`))$r.squared


r_sqr3 = c(0.1303977, 0.05933941, 0.1090481, 0.04772465, 0.08428853, 0.1108832, 0.06154308)


shotperc_rsqr_table <- data.frame(Response, Explanatory, r_sqr3) |> 
  rename(`ShotPerc_R^2` = r_sqr3)



rsqr_table <- cbind(EVG_rsqr_table, EVA_rsqr_table, shotperc_rsqr_table)


rsqr_table <- rsqr_table[ , !duplicated(colnames(rsqr_table))]
```
